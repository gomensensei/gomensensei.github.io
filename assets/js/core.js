...
function shouldSkipGlobalVisualEffect(target) {
    if (!target || !target.tagName) return false;
    const blockedTags = ['INPUT', 'BUTTON', 'SELECT', 'A', 'LABEL', 'TEXTAREA'];
    return blockedTags.includes(target.tagName);
}
document.addEventListener('mousedown', (e) => {
    if (shouldSkipGlobalVisualEffect(e.target)) return;
